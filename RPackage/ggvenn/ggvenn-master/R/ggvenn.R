...
#' @param text_family Text font family for intersect contents.
...
ggvenn <- function(data, columns = NULL,
  ...
  if (nrow(venn$labels) > 0) {
    n = nrow(venn$labels)
    if (n == 2) {
      venn$labels$hjust <- c(1, 0)  # 右对齐和左对齐
    } else if (n == 3) {
      venn$labels$hjust <- c(1, 0, 0.5)  # 右对齐，左对齐和居中对齐
    } else if (n == 4) {
      venn$labels$hjust <- c(1, 1, 0, 0)  
    } else {
      venn$labels$hjust <- 0.5  # 默认居中对齐
    }
    g <- g +
